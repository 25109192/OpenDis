...
{
    if (conn[i].num != 2 || nodes[i].constraint == PINNED_NODE || nodes[i].constraint == SURFACE_NODE) return 1;
    /*
...
{   
    if (nodes[i].constraint == PINNED_NODE || nodes[i].constraint == SURFACE_NODE) return i;
...
{
// 拒绝真 PIN(FR 源端点)和 SURFACE_NODE(死码)的合并
    // 注意:INCLUSION_NODE(夹杂表面节点)允许合并,自动通过此判断
    if (nodes[n1].constraint == PINNED_NODE || nodes[n1].constraint == SURFACE_NODE 
        || nodes[n2].constraint == PINNED_NODE || nodes[n2].constraint == SURFACE_NODE) {
        return false;
    }

    // Guard: avoid creating a node with degree > MAX_CONN
    int deg1 = conn[n1].num;
    int deg2 = conn[n2].num;
    if (deg1 + deg2 > MAX_CONN) {
      //  ExaDiS_log("[merge_nodes_position] SKIP merge: deg1=%d deg2=%d would exceed MAX_CONN=%d for n1=%d n2=%d\n",
      //             deg1, deg2, MAX_CONN, n1, n2);
        return false;
    }
    ...
    for (int i = 0; i < nodelist.size(); i++) {
        if (conn[nodelist[i]].num > 0) {
            ...
            printf(" Warning: removing %d connections for node %d\n",
                conn[nodelist[i]].num, nodelist[i]);
        }
    }
